...
export class AggregatedCalendarService implements IAggregatedCalendarService {
   ...
   * @memberof AggregatedCalendarService
   */
  public getEventsForCalendar(calendarRestApi: string, calendarColor: string, startDate: string, endDate: string, categoryValue: string): Promise<any[]> {
    return new Promise<FullCalendarEvent[]>((resolve, reject) => {
      ...
      {
        let cateFilter = ' and (Category eq \''+ categoryValue +'\')';
      }
      ...
      let _webRestApi: string = calendarRestApi +
        ...
        + startDate + '\' and EventDate le \'' + endDate + '\'))' + cateFilter;
